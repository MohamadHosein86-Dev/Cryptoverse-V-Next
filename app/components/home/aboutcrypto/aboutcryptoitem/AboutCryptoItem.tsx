...
export default function AboutCryptoItem({ titel, revers = false, description, image }: { titel: string; revers?: boolean; description: string; image: string }) {
  ...
  return (
    ...
    >
      <div className="max-sm:w-full text-white md3:w-1/2 flex flex-col gap-2 md:order-1">
        <h3 className="text-2xl font-bold">{titel}</h3>
        <p className="text-sm leading-7 sm:text-normal w-10/12 text-snow text-justify">{description}</p>
      </div>
