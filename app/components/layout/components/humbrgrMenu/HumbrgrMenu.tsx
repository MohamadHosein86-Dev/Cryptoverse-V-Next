...
export default function HumbrgrMenu({ svgRef, openSideMenu }: { svgRef: RefObject<SVGSVGElement | null>; openSideMenu: boolean }) {
  return (
    <svg ref={svgRef} style={{ WebkitTapHighlightColor: "transparent" }} className={`ham ham1 absolute cursor-pointer select-none transition-transform duration-400 ${openSideMenu ? "rotate-45" : ""}`} viewBox="0 0 100 100" width={50}>
      <path
        className="fill-none transition-[stroke-dasharray,stroke-dashoffset] duration-400 stroke-white stroke-[4] top"
        style={{
      ...
      />
      <path className="fill-none transition-[stroke-dasharray,stroke-dashoffset] duration-400 stroke-white stroke-[4] middle" d="m 30,50 h 40" strokeLinecap="round" />
      <path
        className="fill-none transition-[stroke-dasharray,stroke-dashoffset] duration-400 stroke-white stroke-[4] bottom"
        style={{
